...
function initializeDataTableforFiles() {
        ...
        columns: [
            ...
            { data: "status" },
            {
                ...
                render: function (data) {
                    if (data.file_path) {
                        return `<button class="btn btn-sm btn-success download-file">Download</button>\n                        <button class="btn btn-sm btn-secondary preview-file">Preview</button>`;
                    }
                    return 'No File';
                }
            },
            {
                ...
                defaultContent: `
                    ...
                    <button class="btn btn-sm btn-danger delete-file"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash3" viewBox="0 0 16 16"><path d="M6.5 1h3a.5.5 0 0 1 .5.5v1H6v-1a.5.5 0 0 1 .5-.5zm4.5 1.5v-1A1.5 1.5 0 0 0 9.5 0h-3A1.5 1.5 0 0 0 5 1.5v1H1.5a.5.5 0 0 0 0 1h.538l.853 10.66A2 2 0 0 0 4.885 16h6.23a2 2 0 0 0 1.994-1.84l.853-10.66h.538a.5.5 0 0 0 0-1zm1.958 1-.846 10.58a1 1 0 0 1-.997.92h-6.23a1 1 0 0 1-.997-.92L3.042 3.5zm-7.487 1a.5.5 0 0 1 .528.47l.5 8.5a.5.5 0 0 1-.998.06L5 5.03a.5.5 0 0 1 .47-.53Zm5.058 0a.5.5 0 0 1 .47.53l-.5 8.5a.5.5 0 1 1-.998-.06l.5-8.5a.5.5 0 0 1 .528-.47ZM8 4.5a.5.5 0 0 1 .5.5v8.5a.5.5 0 0 1-1 0V5a.5.5 0 0 1 .5-.5Z"/></svg>
                    </button>`
            }
        ]
    });
    ...
    $('#file-table tbody').on('click', 'button.download-file', function () {
        const row = $(this).closest('tr');
        const data = $('#file-table').DataTable().row(row).data();
        window.open(`http://localhost:49200/api/download-file/${data.entry_id}`, '_blank');
    });

    $('#file-table tbody').on('click', 'button.preview-file', function () {
        const row = $(this).closest('tr');
        const data = $('#file-table').DataTable().row(row).data();
        window.open(`http://localhost:49200/api/preview-file/${data.entry_id}`, '_blank');
    });
...
function setupFileModalActions() {
    ...
    $('#save-file').off('click').on('click', function () {
        ...
        formData.append('file_number', $('#file_number').val());
        formData.append('subject', $('#subject').val());
        formData.append('officer_assigned', $('#officer_assigned').val());
        formData.append('status', $('#status').val());
        formData.append('recieved_date', $('#recieved_date').val());
        formData.append('date_sent', $('#date_sent').val());
        formData.append('file_type', $('#file_type').val());
        formData.append('reciepient', $('#reciepient_name').val());
